...
```shell
cat > hello.c <<'CODE' 或者 cat > hello.c <<"CODE" 或者cat > hello.c <<EOF 都是可以的
# 不过有区别：
<<'CODE'
<<"CODE"
# 有引号，里面的 $变量 等不会被 Shell 展开。
# 没有引号，Shell 会对里面的一些内容进行展开。
# 例如
name=Tom
cat <<EOF # 没有引号会展开$变量
hello $name
...
> ```
>
> 所以“进程是资源分配的重要实体”，你现在可以简单理解成：
>
> > **操作系统以进程为重要单位，管理程序运行需要的各种资源。**

> **调度又是什么？**
>
> 假设你的电脑只有一个 CPU 核心，但现在：
>
> ```
> Chrome 想用 CPU
> Nginx 想用 CPU
...
> ```
>
> 这个“**决定谁什么时候使用 CPU**”的过程，就叫**调度（Scheduling）**。
>
> 所以：
>
> > **调度 = 操作系统安排谁使用 CPU。**
...
例如 CPU 最近一直需要某些数据，就把它们放进 Cache：
CPU：我要数据 A
 ↓
Cache：我这里有！
 ↓
...
> 不考虑缓存（cache）时，CPU 不能直接读写键盘、显示器、硬盘等外设的数据。CPU 主要直接和内存打交道；外设输入输出数据，也要先写入内存或从内存读取。
...
![image-20260903211228294](4. 冯诺依曼体系.assets/image-20260903211228294.png)
...
注意：`ps -ef` 和 `ps aux`查所有进程，两种都常用；想看父进程关系常用 `ps -ef`，想看 CPU/内存/状态常用 `ps aux`。

| 命令     | 更方便观察什么                                         |
| -------- | ------------------------------------------------------ |
| `ps -ef` | **父子进程关系**，因为直接显示 `PPID`                  |
| `ps aux` | **CPU、内存、进程状态**，因为有 `%CPU`、`%MEM`、`STAT` |
...
>    ```
>
>    CPU 实际上就是一条一条执行这些最终转换后的机器指令。
>
> 3. **控制数据流转**
>    决定什么时候从内存取数据、什么时候写回内存、什么时候调用操作系统、什么时候和外设配合。

标准答案：

键盘输入的数据先被操作系统驱动（准确来说是指 **设备驱动程序**，驱动程序 = 操作系统用来控制具体硬件的一层软件。完整是你按下键盘 → 键盘硬件产生数据 → 操作系统中的键盘驱动处理这些硬件数据 → 操作系统把处理后的输入暂时保存在内存中的缓冲区 → 程序再读取。）处理，进入内存缓冲区；聊天软件进程读取输入内容并在 CPU 上处理；点击发送后，聊天软件调用系统接口把数据交给操作系统网络协议栈；协议栈封装数据后放入发送缓冲区，网卡从内存中取数据发送到网络；对方设备接收后，网卡和操作系统把数据放入内存，对方聊天软件读取并显示到屏幕。
...
> **操作系统给每个进程提供的一套属于它自己的内存地址世界。**

现代操作系统里，我们通常说的是**虚拟地址空间**。
...
程序里的代码、变量、堆、栈等都会安排在这个地址空间中。

------

**3. 什么是虚拟地址？**

程序使用的地址通常不是直接的物理 RAM 地址，而是**虚拟地址**。
...
> **虚拟地址空间只是可使用的“地址编号范围”，不是一块真的存储空间。**

它很大，不代表占用了同样大小的 RAM 或硬盘。

不过管理这些地址映射的**页表等数据结构会占少量真实内存**。
...
![image-20260904150118666](4. 冯诺依曼体系.assets/image-20260904150118666.png)
...
> `fork` 位于 man 手册的**第 2 章：系统调用 System Calls**。

Linux 的 man 被分成很多 section，你现在常见的先记三个：
...
```shell
write(1, "hello syscall\n", 14) = 14 # = 后面的是返回值：表示实际上成功写了 14 个字节。
...
- 你在 C 代码里写的是 `printf()`。
- 但真正向终端输出时，底层可能调用了 `write()` 系统调用（**用户程序请求 Linux 内核帮自己做事情的接口。**）。
- `1` 表示标准输出 stdout。
- `"hello syscall\n"` 是写出的内容。
- `14` 是写出的字节数。
...
> 操作系统对下管理硬件和软件资源，比如 CPU、内存、文件、设备；对上为应用程序提供统一接口和良好运行环境，比如系统调用、文件抽象、进程抽象等(**抽象 = 操作系统把底层复杂的东西包装起来，给程序一个简单、统一的使用方式。**)。
...
![image-20260904161758965](4. 冯诺依曼体系.assets/image-20260904161758965.png)

```shell
-rwxr-xr-x 1 root root 16000  9月 4 16:17 loop*
                                              ↑
                                          *：可执行文件，这个文件具有可执行权限
```
...
```bash
ps -ef | grep sleep_loop | grep -v grep # grep -v grep 中的 -v 表示：反向匹配：把“包含指定内容”的行排除掉，只留下“不包含”的行。
```
...
![image-20260904163535567](4. 冯诺依曼体系.assets/image-20260904163535567.png)
...
里面根本没有 `loop` 这个单词，所以：没有标题这一行

让 `grep` **同时匹配标题和 `loop`**：

```shell
-E           使用扩展正则表达式
grep -E 'USER|loop'  # 匹配包含 USER 或者包含 loop 的行。
USER|loop    匹配 USER 或者 loop # -E 开启扩展正则表达式，这时候 | 表示“或者”。
因为标题行包含 USER：
USER   PID  %CPU %MEM   VSZ   RSS  TTY  STAT  START  TIME  COMMAND
↑

# 如果你真正想找的是你运行的 ./loop，可以匹配得更精确
ps aux | grep -E 'USER|\./loop' | grep -v grep # 在正则表达式中.符号表示任意一个字符
# 如果写grep -E '.loop' 即：任意一个字符 + loop，因此别把这个 . 当特殊符号，我就是要匹配真正的 .。
# 因此要给 . 加一个反斜杠，此时这里的 . 就是真正的句号 .。

此外'USER|loop'这里的单引号相当于告诉 Shell：
这一整坨 'USER|loop' 是一个参数，不要把里面的 | 当 Shell 管道处理。
grep -E "USER|\./loop" 双引号这里也可以，但更加推荐单引号。
```
...
- `ps` 等工具很多信息也来自 `/proc`。
- **Linux 内核专门提供的一个“实时信息窗口”，让用户可以通过文件的形式查看系统和进程当前的运行状态。**
- `/proc` 是一个**虚拟文件系统**（procfs）。

**`/proc/88465` 到底是什么意思？**

Linux 中，每个正在运行的进程，通常都会在 `/proc` 下有一个**以 PID 命名的目录**：

```
进程   PID                /proc

loop  PID=88465  →   /proc/88465/
bash  PID=88292  →   /proc/88292/
```

![image-20260904165537218](4. 冯诺依曼体系.assets/image-20260904165537218.png)

```
/proc/88465/
│
├── status      进程状态
├── cmdline     启动这个进程时的命令行参数
├── exe         指向可执行程序
├── cwd         当前工作目录
├── fd/         打开的文件描述符
├── maps        进程虚拟内存映射
...
![image-20260904170504908](4. 冯诺依曼体系.assets/image-20260904170504908.png)



---

### 3.6.6 用 pstree 查看进程树：看进程之间“谁是谁的父进程”
...
```bash
pstree -p $$ # 以当前 shell $$ 为根，向下查看它的子进程。（不同shell的根不一样）
pstree -p 88292
# 从 PID 88292 这个进程开始，显示它下面的进程树。
```
...
![image-20260904171751956](4. 冯诺依曼体系.assets/image-20260904171751956.png)

---

### 3.6.7 用 pgrep 查找进程：根据进程名字直接找 PID
...
```shell
root@zmm:~/linux_os/26_9_3/linux_process_note/charpter1# pgrep loop -lx
88465 loop
root@zmm:~/linux_os/26_9_3/linux_process_note/charpter1# 
```
...
```
       ...
       ┌─────────────────┼──────────────────┐
       ↓                 ↓                  ↓
     ps -ef           pstree -p          pgrep
       ↓                 ↓                  ↓
看 PID/PPID         看父子关系          根据名字找 PID
...
| 命令             | 参数    | 参数含义                                           | 主要用途                             |
| ---------------- | ------- | -------------------------------------------------- | ------------------------------------ |
| `ps -ef`         | `-e`    | **every**，显示所有进程                            | 查看系统所有进程                     |
|                  | `-f`    | **full-format**，完整格式                          | 显示 PID、**PPID**、启动时间、命令等 |
| `ps aux`         | `a`     | 显示其他用户的进程，也包括有终端的进程             | 扩大查看范围                         |
|                  | `u`     | **user-oriented format**，以用户友好的详细格式显示 | 显示 USER、**%CPU、%MEM** 等         |
|                  | `x`     | 包括**没有控制终端（TTY）**的进程                  | 可以看到很多后台/系统进程            |
| `pstree -p`      | `-p`    | **PID**                                            | 用进程树显示父子关系，同时显示 PID   |
| `pgrep -l 名字`  | `-l`    | **list-name**                                      | 找进程时同时显示 PID 和进程名        |
| `pgrep -x 名字`  | `-x`    | **exact**                                          | 要求进程名完整匹配                   |
| `pgrep -lx 名字` | `-l -x` | 显示名字 + 精确匹配                                | **精确查找某个进程并显示 PID+名字**  |
...
```bash
root@zmm:~/linux_os/26_9_4/linux_process_note/chapter2# cat > get_pid.c << 'EOF'
> #include <stdio.h>
> #include <sys/types.h>
> #include <unistd.h>
> 
...
> }
> EOF
root@zmm:~/linux_os/26_9_4/linux_process_note/chapter2# gcc get_pid.c -o get_pid
root@zmm:~/linux_os/26_9_4/linux_process_note/chapter2# ls
get_pid  get_pid.c
```
...
```text
root@zmm:~/linux_os/26_9_4/linux_process_note/chapter2# ./get_pid 
pid = 88877
ppid = 88292
root@zmm:~/linux_os/26_9_4/linux_process_note/chapter2# 
```
...
PPID = 88322
     ...
     ├── printf(...)
     │
     ├── sleep(30)   ← 这30秒进程依然存在！只是暂时处于睡眠状态，不需要占用 CPU 执行。
     │
     │
     └── return 0
             ↓
...
Linux 不会因此强制把这个子进程也杀掉。

系统会让它被另一个合适的进程接管（reparent），所以之后你再查询它的 PPID，可能就变成了新的父进程 PID。

因此你刚才问的知识点，核心其实只有一句：

> **父进程创建子进程后，父子是两个独立存在的进程；父进程可以先退出，子进程也可以继续运行。**
...
![image-20260904151846728](4. 冯诺依曼体系.assets/image-20260904151846728.png)
...
实际上 Linux 为了提高效率，`fork()` 通常还会利用 **Copy-on-Write（写时复制）**，不是傻乎乎地立刻把父进程所有物理内存复制一遍，这个等你继续学 `fork` 时再理解就行。
...
**Copy-on-Write（COW）= 写时复制**。核心思想一句话：

> **先不复制，让父子进程暂时共享同一份物理内存；等谁真的要修改某块内存时，再给它复制一份，让它自己修改。**
...
尤其如果子进程刚创建出来马上就去执行另一个程序，那前面的 500MB 复制可能全白干了。
...
注意这里说的是**物理内存共享**。从程序自己的视角来看，父子进程仍然拥有各自独立的虚拟地址空间。
...
假如最后子进程只修改了其中很少一部分，那么就完全没必要复制全部 500MB。

所以它的思想其实特别简单：

> **“你不改，我就不给你复制；等你真要改的时候，我再复制。”**

------

**6. 还有一个细节：不是修改一个 `int` 就只复制 4 字节**
...
> `fork()` 创建出来的子进程拥有父进程地址空间的副本。

千万不要立即理解成：

> `fork()` 一执行，Linux 就把父进程所有物理内存完整 `memcpy` 一遍。
...
你现在可以把 **COW** 记成一句非常形象的话：

> **fork 后先“共用”，谁要“动笔修改”，就给谁复印一份再让他改。**
